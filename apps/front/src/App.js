...
const LoginView = ({ onSubmit, onSignUp, error }) => {
    ...
    return (_jsxs("main", { style: { maxWidth: "400px", margin: "4rem auto", fontFamily: "Inter, sans-serif" }, children: [_jsx("h1", { children: "Entrar" }), _jsx("p", { children: "Use as credenciais do Supabase Auth para acessar o workspace." }), _jsxs("form", { onSubmit: handleSubmit, style: { display: "flex", flexDirection: "column", gap: "0.75rem" }, children: [_jsxs("label", { children: ["Email", _jsx("input", { type: "email", value: email, onChange: (event) => setEmail(event.target.value), required: true })] }), _jsxs("label", { children: ["Senha", _jsx("input", { type: "password", value: password, onChange: (event) => setPassword(event.target.value), required: true })] }), _jsx("button", { type: "submit", disabled: submitting, children: submitting ? "Processando..." : isRegistering ? "Cadastrar" : "Entrar" }), _jsx("button", { type: "button", onClick: () => setIsRegistering((prev) => !prev), style: { background: "transparent", border: "none", color: "#3b82f6", cursor: "pointer" }, children: isRegistering ? "Já tenho conta" : "Criar nova conta" }), error && _jsx("span", { style: { color: "red" }, children: error })] })] }));
};
    ...
    useEffect(() => {
        ...
        const loadOrganizations = async () => {
            try {
                ...
                if (!data.organizations.find((org) => org.id === selectedOrganizationId)) {
