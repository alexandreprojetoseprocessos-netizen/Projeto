...
export const config = {
  ...
  supabase: {
    ...
    db: {
      host: parsed.data.SUPABASE_DB_HOST ?? null,
      port: Number(parsed.data.SUPABASE_DB_PORT),
      name: parsed.data.SUPABASE_DB_NAME ?? null,
      user: parsed.data.SUPABASE_DB_USER ?? null,
      password: parsed.data.SUPABASE_DB_PASSWORD ?? null
    }
  },
